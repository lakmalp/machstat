...
import { faEyeSlash, faSquarePlus, faTrashCan } from "@fortawesome/free-regular-svg-icons";
...
export default function Header({ columns, moveColumn, hideColumn, showColumn, visibleColumns }) {
    ...
    return (
        <>
            <div className={'w-full flex items-center bg-gray-501 rounded-lg1 mb-1 h-12 shadow1 ' + (columnChooserOn ? "" : "hidden")}>
                                ...
                                return (
                                    <div key={i} className="mr-4">
                                        <button
                                            ...
                                            className={" font-dm-sans text-xs shadow rounded py-1 px-2 " + ((12 - visibleColSpanSum) >= (columns.filter(col => col.name === column.name)[0].colSpan) ? "hover:bg-gray-50 bg-gray-100" : "text-gray-400")}
                                        >
                                            <FontAwesomeIcon icon={faSquarePlus} className=" fa-sm pr-2 " /> 
                                            {columns.filter(col => col.name === column.name)[0].label}
                                        </button>
                                    </div>
                                )
                            })
            ...
            <div className='w-full flex items-center bg-gray-100  rounded-t-lg1 h-8 pt-1'>
                <div className='w-12 flex items-center justify-center'>
                ...
                <div className="w-full grid grid-cols-12 gap-4 items-center bg-gray-1001 rounded-tr-lg pr-2">
                    {
                        ...
                        visibleColumns
                            .map((column, i) => {
                                ...
                                return (
                                    <div key={i} className={` font-dm-sans col-span-${columns.filter(col => col.name === column)[0].colSpan} flex items-center`}>
                                        <div key={column} className={`text-${columns.filter(col => col.name === column)[0].textAlign}  text-xs text-gray-700 `}>{columns.filter(col => col.name === column)[0].label}</div>
                                        <div className="flex items-center ml-2">
                                            ...
                                            <FontAwesomeIcon icon={faEyeSlash} className=" text-gray-300 hover:text-gray-500 fa-xs ml-2" onClick={() => hideColumn(column)} />
                                        </div>
                                    </div>
                                )
                            })}
